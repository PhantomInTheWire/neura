...
# Assuming models are now in py_neuro package
...
if not GEMINI_API_KEY:
    ...
    try:
        ...
        logger.info("Gemini configured successfully with {gemini_model}")
...
def extract_text_and_images_pdf(file_path: str, request_image_dir: str) -> tuple[str, List[models.ExtractedImageInfo]]:
    ...
    text = ""
    extracted_images_info: List[models.ExtractedImageInfo] = []
    try:
            ...
            for img_index, img in enumerate(image_list):
                ...
                try:
                    ...
                    if pre_filter_image(image_save_path):
                         extracted_images_info.append(models.ExtractedImageInfo(filename=image_filename, page_number=page_num + 1))
                    else:
                         # Optionally delete the filtered-out image file immediately
                         try: os.remove(image_save_path)
                         except OSError: pass # Ignore error if file already gone
                    img_counter += 1
                except Exception as img_extract_err:
                     logger.warning(f"Could not extract image ref {xref} on page {page_num+1}: {img_extract_err}")

        logger.info(f"Extracted text and {len(extracted_images_info)} potentially relevant images from PDF: {file_path}")
        doc.close()
        return text, extracted_images_info
    except Exception as e:
...
def extract_text_and_images_pptx(file_path: str, request_image_dir: str) -> tuple[str, List[models.ExtractedImageInfo]]:
    ...
    try:
            ...
            for shape in slide.shapes:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                    try:
                        ...
                        if pre_filter_image(image_save_path):
                            extracted_images_info.append(models.ExtractedImageInfo(filename=image_filename, page_number=slide_num + 1))
                        else:
                            try: os.remove(image_save_path)
                            except OSError: pass
                        img_counter += 1
        ...
        logger.info(f"Extracted text and {len(extracted_images_info)} potentially relevant images from PPTX: {file_path}")
        return text, extracted_images_info
    except Exception as e:
...
""",
    ...
    try:
        ...
        class TempWrapper(models.BaseModel):
             # Use the correct model name: StudyGuideSection
...
# Update the response model for the endpoint decorator
@router.post("/upload", response_model=models.StudyGuideResponse)
async def create_hierarchical_study_guide(file: UploadFile = File(...)):
    """
    ...
    try:
        if extension == ".pdf":
            # extract_text_and_images now returns only filtered images
            text_content, filtered_images_info = extract_text_and_images_pdf(temp_file_path, request_image_dir)
            # We need a way to know *all* images for the final response,
            # maybe the extraction function should return both lists?
            # For now, let's assume all_extracted = filtered for simplicity, needs refinement
            all_extracted_images_info = filtered_images_info
        elif extension == ".pptx":
            text_content, filtered_images_info = extract_text_and_images_pptx(temp_file_path, request_image_dir)
            all_extracted_images_info = filtered_images_info # Same assumption as above
        elif extension == ".docx":
        ...
        # The function now returns List[models.StudyGuideSection]
